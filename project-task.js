...
let animals = [];
let fees = [];
//Added a conditional of pet name can't be an empty string, and the fee must be a number
function addAnimal(name, fee) {
    if (!name || (name.trim() === "") || fee < 0 || isNaN(fee)) {
...
while (true) {
    try {
        ...
        //Adding a new try and catch for addAnimal to pinpoint errors
            try {
       ...
       //Adding new try and catch with getAdoptionFee to pinpoint errors
            try {
                let fee = getAdoptionFee(animal);
                console.log(`${animal}'s adoption fee is $${fee}.`);
        
            } catch (err) {
                console.log("Error finding fee:", err.message);
            }
